/// Franklyn Gonzalez, COP3502, Computer Science I, last edited July 28th, 2020, 22:48
// ***
// ***
// ***
...
/**
   A simple wrapper function that clears the pointer array in a Task.
 */
...
/**
   A wrapper to free up the memory allocated for a BinaryHeap
 */
...
/**
   A convenience function for swapping two Tasks in an heap's internal array
 */
...
/**
   If a node is larger than its parent, "bubbles" it up until its parent is larger than it
 */
...
/**
   Inserts a new task into the heap, up-heaping and extending it as needed
 */
...
Task readTask(int id)
{
    ...
    task.phaseLength = malloc(sizeof(int) * task.numPhases);
...
int main(void)
{
    ...
    Task* task = malloc(sizeof(Task) * numTasks);
    int i;
